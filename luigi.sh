...
# Neutron Clang is newer than the toolchain the upstream tree was written
# against and its -Wuninitialized is stricter, so it turns a real latent bug
# in sde_rm.c into a hard -Werror failure. Patch it idempotently so it
# applies on every checkout without hand-editing the source each time.
quirks() {
	local f="${KDIR}/techpack/display/msm/sde/sde_rm.c"
	if [ -f "${f}" ] && grep -q 'const struct sde_pingpong_cfg \*pp_cfg;' "${f}"; then
